...
static void belle_sip_message_clone(belle_sip_message_t *obj, const belle_sip_message_t *orig) {
	...
	for (l = orig->header_list; l != NULL; l = l->next) {
		...
		if (c->header_list) {
			belle_sip_list_t *ll =
			    belle_sip_list_copy_with_data(c->header_list, (void *(*)(void *))belle_sip_object_clone);
...
BELLE_SIP_INSTANCIATE_VPTR(
    belle_sip_message_t, belle_sip_object_t, belle_sip_message_destroy, belle_sip_message_clone, NULL, TRUE);
...
headers_container_t *belle_sip_headers_container_get(const belle_sip_message_t *message, const char *header_name) {
	belle_sip_list_t *result = belle_sip_list_find_custom(
	    message->header_list, (belle_sip_compare_func)belle_sip_headers_container_comp_func, header_name);
...
void belle_sip_message_add_first(belle_sip_message_t *message, belle_sip_header_t *header) {
	...
	headers_container->header_list =
	    belle_sip_list_prepend(headers_container->header_list, belle_sip_object_ref(header));
}
...
void belle_sip_message_add_headers(belle_sip_message_t *message, const belle_sip_list_t *header_list) {
	...
	for (; header_list != NULL; header_list = header_list->next) {
		...
		if (strcmp(belle_sip_header_get_name(h), hname) != 0) {
			belle_sip_fatal(
			    "Bad use of belle_sip_message_add_headers(): all headers of the list must be of the same type.");
			return;
		}
...
void belle_sip_message_set_header(belle_sip_message_t *msg, belle_sip_header_t *header) {
	...
	headers_container->header_list =
	    belle_sip_list_free_with_data(headers_container->header_list, belle_sip_object_unref);
...
belle_sip_object_t *_belle_sip_message_get_header_by_type_id(const belle_sip_message_t *message,
...
/*
belle_sip_error_code belle_sip_message_named_headers_marshal(belle_sip_message_t *message, const char* header_name,
char* buff, size_t buff_size, size_t *offset) { belle_sip_error_code error=BELLE_SIP_OK; belle_sip_list_t* header_list =
belle_sip_message_get_headers(message,header_name); if (!header_list) { belle_sip_error("headers [%s] not
found",header_name); return 0;
    }
    for(;header_list!=NULL;header_list=header_list->next){
        belle_sip_header_t *h=BELLE_SIP_HEADER(header_list->data);
...
static void belle_sip_message_for_each_header(const belle_sip_message_t *message, each_header_cb cb, void *user_data) {
	...
	for (headers_list = message->header_list; headers_list != NULL; headers_list = headers_list->next) {
		for (header_list = ((headers_container_t *)(headers_list->data))->header_list; header_list != NULL;
...
static void belle_sip_request_clone(belle_sip_request_t *request, const belle_sip_request_t *orig) {
	...
	if (orig->uri)
		request->uri = (belle_sip_uri_t *)belle_sip_object_ref(belle_sip_object_clone((belle_sip_object_t *)orig->uri));
	if (orig->absolute_uri)
		request->absolute_uri = (belle_generic_uri_t *)belle_sip_object_ref(
		    belle_sip_object_clone((belle_sip_object_t *)orig->absolute_uri));
	if (orig->rfc2543_branch) request->rfc2543_branch = belle_sip_strdup(orig->rfc2543_branch);
}

belle_sip_error_code
belle_sip_request_marshal(belle_sip_request_t *request, char *buff, size_t buff_size, size_t *offset) {
	belle_sip_error_code error =
	    belle_sip_snprintf(buff, buff_size, offset, "%s ",
	                       belle_sip_request_get_method(request) ? belle_sip_request_get_method(request) : "");

	if (error != BELLE_SIP_OK) return error;
	if (request->uri) error = belle_sip_uri_marshal(belle_sip_request_get_uri(request), buff, buff_size, offset);
...
void belle_sip_request_set_uri(belle_sip_request_t *request, belle_sip_uri_t *uri) {
	...
	if (request->absolute_uri && uri) {
		belle_sip_warning("absolute uri [%p] already set for request [%p], cleaning it", request->absolute_uri,
		                  request);
...
belle_sip_header_t *belle_sip_message_get_header(const belle_sip_message_t *msg, const char *header_name) {
	...
	if (l != NULL) return (belle_sip_header_t *)l->data;
	return NULL;
}
...
void belle_sip_message_set_body_handler(belle_sip_message_t *msg, belle_sip_body_handler_t *body_handler) {
	const belle_sip_header_content_length_t *content_length_header =
	    belle_sip_message_get_header_by_type(msg, belle_sip_header_content_length_t);
	const belle_sip_header_content_type_t *content_type_header =
	...
	if (body_handler) body_handler_headers = belle_sip_body_handler_get_headers(body_handler);
	...
	if (body_handler != NULL) {
		if (BELLE_SIP_OBJECT_IS_INSTANCE_OF(body_handler, belle_sip_multipart_body_handler_t)) {
				...
				if (parts) {
					...
					if (first_part_content_type) {
						char *type_slash_subtype = belle_sip_strdup_printf(
						    "%s/%s", belle_sip_header_content_type_get_type(first_part_content_type),
						    belle_sip_header_content_type_get_subtype(first_part_content_type));
						belle_sip_parameters_set_parameter(BELLE_SIP_PARAMETERS(content_type), "type",
						                                   type_slash_subtype);
					...
					belle_sip_error("Multipart related body handler [%p] cannot be set without first part",
					                body_handler);
				}
			...
			const belle_sip_header_content_type_t *new_content_type_header =
			    belle_sip_message_get_header_by_type(msg, belle_sip_header_content_type_t);
			if (!new_content_type_header ||
			    !belle_sip_parameters_has_parameter(BELLE_SIP_PARAMETERS(new_content_type_header), "boundary")) {
				belle_sip_parameters_set_parameter(
				    BELLE_SIP_PARAMETERS(content_type), "boundary",
				    belle_sip_multipart_body_handler_get_boundary(multipart_body_handler));
			...
			for (; headers != NULL; headers = headers->next) {
				...
				if (strcasecmp(belle_sip_header_get_name(header), BELLE_SIP_CONTENT_LENGTH) == 0 &&
...
static const char *get_multipart_body(belle_sip_message_t *msg) {
	if (!BELLE_SIP_OBJECT_IS_INSTANCE_OF(msg->body_handler, belle_sip_multipart_body_handler_t)) return NULL;
...
const char *belle_sip_message_get_body(belle_sip_message_t *msg) {
	...
	if (BELLE_SIP_OBJECT_IS_INSTANCE_OF(msg->body_handler, belle_sip_memory_body_handler_t)) {
		return (const char *)belle_sip_memory_body_handler_get_buffer(BELLE_SIP_MEMORY_BODY_HANDLER(msg->body_handler));
	}
	const char *body = get_multipart_body(msg);
	if (!body) belle_sip_error("belle_sip_message_get_body(): body cannot be returned as pointer.");
	return body;
}
...
void belle_sip_message_set_body(belle_sip_message_t *msg, const char *body, size_t size) {
	...
	if (body && size)
		bh = (belle_sip_body_handler_t *)belle_sip_memory_body_handler_new_copy_from_buffer(body, size, NULL, NULL);
...
void belle_sip_message_assign_body(belle_sip_message_t *msg, char *body, size_t size) {
	belle_sip_body_handler_t *bh =
	    (belle_sip_body_handler_t *)belle_sip_memory_body_handler_new_from_buffer(body, size, NULL, NULL);
	belle_sip_message_set_body_handler(msg, bh);
}
...
static code_phrase_t well_known_codes[] = {{100, "Trying"},
...
const char *belle_sip_get_well_known_reason_phrase(int status_code) {
	...
	for (i = 0; well_known_codes[i].code != 0; ++i) {
		if (well_known_codes[i].code == status_code) return well_known_codes[i].phrase;
	}
...
belle_sip_error_code
belle_sip_response_marshal(belle_sip_response_t *resp, char *buff, size_t buff_size, size_t *offset) {
	belle_sip_error_code error = belle_sip_snprintf(
	    buff, buff_size, offset, "SIP/2.0 %i %s\r\n", belle_sip_response_get_status_code(resp),
...
static int is_authorized_uri_header(const char *header_name) {
   ...
   Contact (in its dialog usage), Organization, Supported, and User-Agent*/
	return (strcasecmp("From", header_name) != 0 && strcasecmp("Call-ID", header_name) != 0 &&
	        strcasecmp("CSeq", header_name) != 0 && strcasecmp("Via", header_name) != 0 &&
	        strcasecmp("Record-Route", header_name) != 0 && strcasecmp("Accept", header_name) != 0 &&
	        strcasecmp("Accept-Encoding", header_name) != 0 && strcasecmp("Accept-Language", header_name) != 0 &&
	        strcasecmp("Allow", header_name) != 0 && strcasecmp("Contact", header_name) != 0 &&
	        strcasecmp("Organization", header_name) != 0 && strcasecmp("Supported", header_name) != 0 &&
	        strcasecmp("User-Agent", header_name) != 0);
}

belle_sip_request_t *belle_sip_request_create(belle_sip_uri_t *requri,
                                              const char *method,
	...
	if (to)
		belle_sip_message_add_header((belle_sip_message_t *)ret, BELLE_SIP_HEADER(to)); /*to might be in header uri*/
	...
	else belle_sip_object_unref(mf);
   ...
   Mime-Version, and Timestamp.*/
	...
	if (belle_sip_uri_get_header_names(requri)) {
		for (iterator = (belle_sip_list_t *)belle_sip_uri_get_header_names(requri); iterator != NULL;
		     iterator = iterator->next) {
			const char *header_name = (const char *)iterator->data;
			/*1 check header name*/
			if (is_authorized_uri_header(header_name)) {
				belle_sip_header_extension_t *extended_header =
				    belle_sip_header_extension_create(header_name, belle_sip_uri_get_header(requri, header_name));
				if (extended_header) {
...
/*
 * note: we must not assume the request to be well formed because this function may be used to generate 400 Bad request
 * response.
 */
...
void belle_sip_response_fill_for_dialog(belle_sip_response_t *obj, belle_sip_request_t *req) {
	...
	if (rr) belle_sip_message_add_headers((belle_sip_message_t *)obj, rr);
	if (belle_sip_response_get_status_code(obj) >= 200 && belle_sip_response_get_status_code(obj) < 300 && !ct) {
		...
		if (strcmp(method, "INVITE") == 0 || strcmp(method, "SUBSCRIBE") == 0) {
			...
			belle_sip_message_add_header((belle_sip_message_t *)obj,
			                             (belle_sip_header_t *)belle_sip_header_contact_new());
		}
	}
}
...
int belle_sip_response_fix_contact(const belle_sip_response_t *response, belle_sip_header_contact_t *contact) {
	...
	if (strcasecmp(belle_sip_header_via_get_transport(via_header), "UDP") != 0) {
		if (!belle_sip_uri_get_transport_param(contact_uri) ||
		    strcasecmp(belle_sip_uri_get_transport_param(contact_uri),
		               belle_sip_header_via_get_transport(via_header)) != 0) {
...
belle_sip_request_t *belle_sip_request_clone_with_body(const belle_sip_request_t *initial_req) {
	...
	if (initial_req->base.body_handler)
		req->base.body_handler = BELLE_SIP_BODY_HANDLER(
		    belle_sip_object_clone_and_ref((belle_sip_object_t *)initial_req->base.body_handler));
	return req;
}
...
int belle_sip_message_check_headers(const belle_sip_message_t *message) {
	if (BELLE_SIP_OBJECT_IS_INSTANCE_OF(message, belle_sip_request_t)) {
		...
		for (i = 0; mandatory_headers[i].method != NULL; i++) {
			if ((strcasecmp(method, mandatory_headers[i].method) == 0) || (mandatory_headers[i].method[0] == '*')) {
				int j;
				for (j = 0; mandatory_headers[i].headers[j] != NULL; j++) {
					if (belle_sip_message_get_header(message, mandatory_headers[i].headers[j]) == NULL) {
						belle_sip_error("Missing mandatory header [%s] for message [%s]",
						                mandatory_headers[i].headers[j], method);
						return 0;
					}
				}
			}
...
int belle_sip_request_check_uris_components(const belle_sip_request_t *request) {
	...
	for (; iterator != NULL; iterator = iterator->next) {
		...
		if (BELLE_SIP_IS_INSTANCE_OF(header, belle_sip_header_address_t)) {
			...
			if (uri && !belle_sip_uri_check_components_from_context(uri, belle_sip_request_get_method(request),
			                                                        belle_sip_header_get_name(header))) {
